...
function computeDevScore(stats) {
  ...
  const repoScore = safe(
    Math.min(Math.round((stats.totalRepos / 100) * 100), 100),
  );
  ...
  const consistency = safe(
    Math.min(Math.round((activeWeeks / totalWeeks) * 100), 100),
  );
  const impact = safe(
    Math.min(Math.round(Math.log10((stats.totalStars || 0) + 1) * 40), 100),
  );

  const diversity = safe(Math.min((stats.topLanguages?.length || 0) * 14, 100));

  const recentCommits = weekly.slice(-4).reduce((a, b) => a + b, 0);
